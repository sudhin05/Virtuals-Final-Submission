...
from Ocular.virtual_ocular_sudhin import ocular_model
        ...
        if os.path.exists(main_folder_path): #
            ...
            while True:
                ...
                for folder in folders:
                    ...
                    # if folder.split['_'][0][5:] == str(casualty_number): 
                                ...
                                if not os.path.exists(txt_path):
                                    ...
                                    if os.path.exists(txt_path):
                                        if count_dict[folder] <= 50:
                                            ...
                                            count_dict[folder] += 1
                                        elif flag and count_dict[folder] >= 50:
                                            # Run the shell script with the argument
                                            subprocess.Popen(["sh", shell_script_path, f"../../{output_subfolder}"])
                                            flag = False
                                else:
                                    txt_files_number +=1
                        if len(folders) + 2 >= len(visited_folders) and image_number >= (txt_files_number-20):
                            print(f"Lenght of folders : {len(folders)} Visiteed Folders : {len(visited_folders)}")
...
def visualize_txt(image_path,txt_path):
    ...
    # Read and draw the body bounding box
    x1_body, y1_body, x2_body, y2_body = map(int, lines[0].strip().split())
    cv2.rectangle(img, (x1_body, y1_body), (x2_body, y2_body), (255, 0, 0), 2)  # Blue box for body

    # Read and draw the face bounding box
    x1_face_rel, y1_face_rel, x2_face_rel, y2_face_rel = map(int, lines[1].strip().split())
    x1_face = x1_body + x1_face_rel
    y1_face = y1_body + y1_face_rel
    x2_face = x1_body + x2_face_rel
    y2_face = y1_body + y2_face_rel
    cv2.rectangle(img, (x1_face, y1_face), (x2_face, y2_face), (0, 255, 0), 2)  # Green box for face

    # Draw the keypoints
    for line in lines[2:]:
        x_rel, y_rel, _ = map(float, line.strip().split())
        x = int(x1_body + x_rel)
        y = int(y1_body + y_rel)
        cv2.circle(img, (x, y), 5, (0, 0, 255), -1)  # Red circles for keypoints

    # Show the image
    cv2.imshow('Image with Bounding Boxes and Keypoints', cv2.resize(img,(640,480)))
    k = cv2.waitKey(0)
    if k == ord('q'):
...
if __name__=="__main__":
    ...
    check_run_3("images_ocu",pose_model,person_model,face_model,output_folder)
